...
class server2{
	...
	//			System.out.println("123242354"+sentence);
		...
		if((sentence.substring(0,8)).equals("FETCHKEY"))	//usernanme for fetch key
			Username = sentence.substring(9);
		else
			Username = sentence.substring(5);		//save username of send
		if(!users.containsKey(Username)){	//user not found
			return 1;
		}
		if((sentence.substring(0,8)).equals("FETCHKEY")){
